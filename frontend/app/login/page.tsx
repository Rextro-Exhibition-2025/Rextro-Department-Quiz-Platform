...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    if (err) {
      const map: Record<string, string> = {
        AccessDenied:
          "Google sign-in was denied or the account isn't linked. If you just registered, please sign in using the same Google account you used during registration; otherwise try signing in again.",
        OAuthSignin: "OAuth sign-in failed. Please try again.",
        OAuthCallback: "OAuth callback error. Please try again.",
        OAuthAccountNotLinked:
          "This Google account is not linked to a student account. Please register first.",
      };
  ...
  const handleRegisterThenGoogle = async () => {
    ...
    try {
      ...
      const payload = { name: formData.name, studentId, email: formData.email };
      ...
      setError(
        body?.message ||
          `Registration failed: ${resp.statusText || resp.status}`
      );
    } catch (e: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok && responseData?.success) {
        ...
        setError(
          'Registration successful. Please sign in using the same Google account you used to register (use "Sign in with Google").'
        );
      } else {
        setError(
          responseData?.message || res.statusText || "Registration failed"
        );
      }
    } catch (err: any) {
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex justify-between items-center mb-2">
              <div className="text-sm handwritten" style={{ color: "#4A3426" }}>
                {isRegistering ? "Register a new scholar" : "Scholar Login"}
              </div>
              <button
                type="button"
            ...
            {isRegistering ? (
              <>
                ...
                <div>
                  ...
                  <p
                    className="text-xs mb-1 handwritten"
                    style={{ color: "#4A3426" }}
                  >
                    Use a working Gmail address.
                  </p>
                  <input
                    id="email"
                    name="email"
