  ...
  useEffect(() => {
    const checkQuizAvailability = async () => {
      try {
        ...
        const resp = await fetch(`${apiUrl}/quizzes/check-availability`);
        const data = await resp.json();
        setQuizzesAvailable(data?.available ?? false);
      } catch (error: any) {
        console.error(
          "Error checking quiz availability:",
          error?.message ?? error
        );
  ...
  return (
    <div className="min-h-screen relative overflow-hidden flex flex-col justify-center items-center">
      ...
      <div
        ...
        style={{
          background:
            "radial-gradient(circle at center, transparent 0%, rgba(42, 26, 17, 0.4) 100%)",
        }}
      />
      ...
      <div
        className="absolute bottom-10 left-10 w-24 h-24 md:w-32 md:h-32 opacity-15 compass-spin z-0"
      ...
      {/* Content Container */}
      <div
        className={`relative z-10 max-w-5xl mx-auto px-6 py-12 text-center transition-all duration-1000 ${
      ...
      >
        ...
        <div className="relative inline-block mb-10 p-8">
          ...
          <img
            ...
            alt=""
            className="absolute top-0 left-0 w-12 h-12 md:w-16 md:h-16 opacity-80"
          />
          <img
            src="/corner-decoration.svg"
            alt=""
            className="absolute top-0 right-0 w-12 h-12 md:w-16 md:h-16 opacity-80 transform scale-x-[-1]"
          />
          <img
            src="/corner-decoration.svg"
            alt=""
            className="absolute bottom-0 left-0 w-12 h-12 md:w-16 md:h-16 opacity-80 transform scale-y-[-1]"
          />
          <img
            src="/corner-decoration.svg"
            alt=""
            className="absolute bottom-0 right-0 w-12 h-12 md:w-16 md:h-16 opacity-80 transform scale-[-1]"
          />
          ...
          <h1
            ...
            style={{
              ...
              color: "#4a2511",
              textShadow:
                "2px 2px 0px rgba(212, 175, 55, 0.3), 4px 4px 8px rgba(0,0,0,0.3)",
        ...
        <p
          className="text-xl md:text-2xl mb-12 max-w-3xl mx-auto leading-relaxed handwritten font-bold"
          style={{
            color: "#2a1a11",
            textShadow: "0 1px 1px rgba(255,255,255,0.5)",
          }}
        >
          "Dive into an immersive quiz experience where knowledge meets
          competition. Climb the leaderboard, and become the ultimate quiz
          champion."
        </p>
        ...
        <div className="flex flex-col sm:flex-row gap-8 justify-center items-center">
          ...
          >
            ...
            {quizzesAvailable ? "Start Quiz" : "Coming Soon"}
