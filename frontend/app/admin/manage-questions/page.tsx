...
export default function ManageQuestions() {
  ...
  useEffect(() => {
    const checkPublishedStatus = async () => {
      ...
      try {
        const response = await api.get<{ isPublished: boolean }>(
        ...
        if ((error as any)?.response) {
          console.error(
            "Error fetching published status:",
  ...
  const handleTabChange = (index: number) => {
    setSelectedQuizSet(
      (quizSets[index] && String(quizSets[index].quizId)) ||
        (quizSets[0] && String(quizSets[0].quizId)) ||
        ""
    );
  };
  ...
  const handleCreateSet = async () => {
    ...
    try {
      const api = await createAdminApi();
      const resp = await api.post("/quizzes/create-quiz-set", {
      ...
      <div className="max-w-4xl mx-auto relative z-10">
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                style={{
                  fontFamily: "Cinzel, serif",
                  background:
                    "linear-gradient(135deg, #fdf6e3 0%, #e3d5b8 100%)",
                  color: "#4a2511",
          ...
          <div className="flex flex-wrap items-center gap-4">
            ...
            <button
              onClick={() => {
                ...
                const selected = quizSets.find(
                  (s) => String(s.quizId) === String(selectedQuizSet)
                );
                if (!selected) return;
            ...
            <div className="flex-1 min-w-[200px]">
              <label htmlFor="quizSelectHeader" className="sr-only">
                Select Quiz
              </label>
              <select
                id="quizSelectHeader"
                ...
                style={{
                  fontFamily: "Cinzel, serif",
                  background:
                    "linear-gradient(135deg, #FFF8E7 0%, #F4E8D0 100%)",
                  color: "#4a2511",
                  border: "2px solid #704214",
                }}
              >
                {quizSets.map((set) => (
                  <option key={set.quizId} value={String(set.quizId)}>{`${
                    set.name
                  } - ${countsBySet[String(set.quizId)] ?? 0}`}</option>
                ))}
              </select>
            </div>
          </div>
        ...
        {showEditModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center">
            ...
            <div className="bg-white rounded-lg p-6 z-60 w-full max-w-md shadow-lg text-gray-900">
              ...
              />
              {editSetError && (
                <div className="text-sm text-red-800 mb-2">{editSetError}</div>
              )}
              <div className="flex justify-end gap-2">
                <button
                  onClick={() => setShowEditModal(false)}
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      if (updated && typeof updated.quizId !== "undefined") {
                        ...
                        console.error(
                          "Update quiz set - unexpected response:",
                          resp?.data
                        );
                      }
                    } catch (err) {
            ...
            >
              <thead>
                <tr
                  style={{
                    background:
                      "linear-gradient(135deg, #e3d5b8 0%, #d4c5a3 100%)",
                  }}
                >
                  <th
        ...
        <div className="parchment-card rounded-2xl shadow-xl p-8 my-6 relative">
          ...
          <div className="flex items-start space-x-4 mb-6">
            ...
            <div className="flex-1">
              ...
              <p
                ...
                style={{ color: "#4a2511", fontFamily: "Crimson Text, serif" }}
              >
                Follow these steps to manage question sets and add questions.
              </p>
            </div>
          </div>
          ...
          <div
            ...
            style={{
              background:
                "linear-gradient(135deg, rgba(255, 248, 231, 0.6) 0%, rgba(232, 213, 181, 0.6) 100%)",
              border: "2px solid #8b5a2b",
            }}
          >
            <ul
              className="text-sm font-medium space-y-3"
              style={{ color: "#2a1a11", fontFamily: "Crimson Text, serif" }}
            >
              <li className="flex items-start">
                <span className="mr-3 font-bold text-orange-600">•</span>
                <span>
                  <strong>First — create Question Sets:</strong> Click{" "}
                  <strong>"Add Question Set"</strong> to create and save one or
                  more sets on the server.
                </span>
              </li>
              <li className="flex items-start">
                <span className="mr-3 font-bold text-orange-600">•</span>
                <span>
                  <strong>Then — add questions:</strong> After creating or
                  selecting a set, click <strong>"Add New Question"</strong> to
                  add questions to the chosen set.
                </span>
              </li>
              <li className="flex items-start">
                <span className="mr-3 font-bold text-orange-600">•</span>
                <span>
                  <strong>Edit Set:</strong> Use <strong>"Edit Set"</strong> to
                  rename the currently selected set.
                </span>
              </li>
              <li className="flex items-start">
                <span className="mr-3 font-bold text-orange-600">•</span>
                <span>
                  <strong>Select a set:</strong> Use the dropdown to choose
                  which set to view or manage.
                </span>
              </li>
              <li className="flex items-start">
                <span className="mr-3 font-bold text-orange-600">•</span>
                <span>
                  <strong>Publish/Unpublish:</strong> Toggle visibility so
                  quizzes are (un)available to students.
                </span>
              </li>
            </ul>
          </div>
