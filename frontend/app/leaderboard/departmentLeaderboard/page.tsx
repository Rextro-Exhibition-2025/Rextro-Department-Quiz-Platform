...
import {
  ...
  transformApiLeaderboard,
} from "./leaderboardTransformer";
  ...
  useEffect(() => {
    const fetchLeaderboard = async () => {
      try {
        ...
        const url = `${apiUrl}/leaderboard?quizId=${encodeURIComponent(
          quizId
        ...
        try {
          ...
          const quizResp = await api.get(
            `/quizzes/${encodeURIComponent(String(quizId))}`
          );
      ...
      >
        ...
        <div className=" mb-8">
          ...
          <p
            ...
            style={{ color: "#4A3426", fontSize: "1.1rem" }}
          >
            {quizName ? `${quizName} — Quest Rankings` : "Quest Rankings"}
          </p>
        </div>
        ...
        <div className="flex justify-center items-end mb-12 space-x-6">
          {/* Second Place */}
          <div
            className="text-center parchment-card p-8 shadow-lg flex flex-col items-center justify-center w-44 md:w-56 relative"
          ...
          >
            ...
            >
              {(() => {
                ...
                const percent =
                  top?.correctPercentage ??
                  school?.correctPercentage ??
                  (typeof school?.score === "number" ? school.score : null);
            ...
            <div className="text-sm handwritten" style={{ color: "#4A3426" }}>
              {(() => {
                ...
                return typeof top?.totalTimeTaken === "number"
                  ? `${(top.totalTimeTaken / 1000).toFixed(2)}s`
                  : "-";
              })()}
            </div>
          </div>
          ...
          {/* First Place */}
          <div
            className="text-center mb-10 p-10 parchment-card shadow-xl flex flex-col items-center justify-center w-56 md:w-72 relative"
            style={{ borderRadius: "50px" }}
          >
            <img
              src="/corner-decoration.svg"
                    ...
                    {school.students && school.students.length > 0 && (
                      <div className="text-sm text-gray-500 mt-1">
                        {(() => {
                          const top = school.students[0] as any;
                          const attempts =
                            top?.attempts != null ? String(top.attempts) : "-";
                ...
                <div className="mt-4 pt-4 border-t border-orange-200">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-right">
                      ...
                      <div className="text-xs text-gray-500 mt-1">
                        {(() => {
                          const totalMs = selectedSchool.students.reduce(
                            (acc, s) =>
                              acc +
                              (typeof s.totalTimeTaken === "number"
                                ? s.totalTimeTaken
                                : 0),
                            0
                          );
