      ...
      try {
        ...
        const safeFetchJson = async (url: string) => {
          ...
          if (!res.ok) {
            ...
            throw new Error(
              `Request to ${url} failed: ${res.status} ${res.statusText} - ${preview}`
            );
          }
          ...
          if (!contentType.includes("application/json")) {
            const preview = text?.slice(0, 500);
            throw new Error(
              `Expected JSON from ${url} but received content-type: ${contentType} - body preview: ${preview}`
            );
          }
        ...
        const qsJson = await safeFetchJson(`${apiUrl}/quizzes/get-quiz-sets`);
        const sets: QuizSet[] = Array.isArray(qsJson.data) ? qsJson.data : [];
        setQuizzes(sets);

        const requests = sets.map((s) =>
          safeFetchJson(
            `${apiUrl}/leaderboard?quizId=${s.quizId}&limit=5`
          ).then((j) => ({
            id: s.quizId,
    ...
    <div className="min-h-screen p-6 relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        <div className="mb-6">
          ...
          >
            The most distinguished scholars of each quest.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {quizzes.map((quiz) => (
            <Link
              ...
              aria-label={`View leaderboard details for ${
                quiz.name ?? `Quiz ${quiz.quizId}`
              }`}
            >
              <img
              ...
              <div
                ...
                style={{
                  background:
                    "linear-gradient(180deg, #651321 0%, #704214 100%)",
                  borderColor: "#704214",
                }}
              >
                <h2
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div
                          ...
                          style={{
                            background:
                              "linear-gradient(135deg, #F4E8D0 0%, #E8D5B5 100%)",
                        ...
                        <div>
                          <div
                            className="font-bold leading-tight max-w-[12rem] truncate"
                            style={{
                              color: "#2C1810",
                          ...
                          >
                            {item.name ?? String(item.studentId)}
                          </div>
                      ...
                      <div className="text-right">
                        ...
                        >
                          {item.correctPercentage != null
                            ? formatPercent(item.correctPercentage)
