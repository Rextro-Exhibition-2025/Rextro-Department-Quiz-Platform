...
function countCorrectAnswers(
  ...
  correctAnswers: { questionId: number; correctOption: string }[]
): number {
  let correctCount = 0;
  submittedAnswers.forEach((sub) => {
    const correct = correctAnswers.find(
      (ca) => ca.questionId === sub.questionId
    );
    if (
      correct &&
      correct.correctOption.toLowerCase() === sub.answer.toLowerCase()
    )
      correctCount++;
  });
  return correctCount;
...
export const checkQuizzesPublishedStatus = async (
  req: Request,
  res: Response
) => {
  try {
